...
class ImporterService
{
	...
	public function importMovies(): bool
	{
		...
		foreach ($movies as $movie) {
			$movieFromDb = $this->movieRepository->find($movie['id']);
			if ($movieFromDb) {
				$movieDetails = Importer::getMovie($movie['id']);
				$this->movieRepository->update($movieFromDb, [
		...
		$chunks = $this->movieRepository->get()->sortByDesc('vote_average')->chunk(210);
		$chunks->shift();
		foreach ($chunks as $chunk) {
			foreach ($chunk as $element) {
				$this->movieRepository->delete($element);
			}
		}
